...
export const createCnpjLookup = (
    ...
    cnpjFieldName: "inscricao" | "cpfCnpj" = "inscricao",
) => {
    return async () => {
        const cnpjValue = form.getValues(cnpjFieldName);
        ...
        // --- Tentativa 1: BrasilAPI (mais permissiva com CORS) ---
        try {
            toast({ title: "Buscando CNPJ...", description: "Consultando BrasilAPI..." });
        ...
        if (!success) {
            try {
                toast({ title: "Tentando outra fonte...", description: "Consultando ReceitaWS..." });
